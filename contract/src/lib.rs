...
mod lottery {
    ...
    pub fn create_lottery(ctx: Context<CreateLottery>, ticket_price: u64) -> Result<()> {
        ...
        // What is a lottery account? -> it holds the id , the winning address,
        let lottery = &mut ctx.accounts.lottery; // -> getting the lottery object
    ...
    pub fn pick_winner(ctx: Context<PickWinner>, lottery_id: u32) -> Result<()> {
        // select a radom ticket as winner and set the winner_id to that winner
        let lottery = &mut ctx.accounts.lottery;
    ...
    pub fn claim_prize(ctx: Context<ClaimPrize>, _lottery_id: u32, _tick_id: u32) -> Result<()> {
        ...
        // Transfer the prize from the lottery account
